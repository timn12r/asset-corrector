...
def main():
    ...
    #Update local dictionary from Asset attributes
    if attributes:
    ...
    if all(value is None for value in attribute_dict.values()):
        print('Attributes not found! Invalid UID/Category? Exiting...')
        exit()
    ...
    def updateAttribute(attr, value):
        for attribute in attributes:
            if attribute.get('typeName') == f'{attr}':
    ...
    if attribute_dict['CPU Type'] is not None:
        ...
        updateAttribute('CPU Type', cleaned_cpu)
    else:
    ...
    if attribute_dict['Battery Wear Level'] is not None:
        ...
        updateAttribute('Battery Wear Level', cleaned_battery)
    else:
